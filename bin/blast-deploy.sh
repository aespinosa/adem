...
do
  ...
  if [  -f $DIR/tmp/blast-$SITE.sh ]; then
     rm -f $DIR/tmp/blast-$SITE.sh
  fi
  echo "#/bin/sh" >> $DIR/tmp/blast-$SITE.sh
  echo "cd  $APP/$VO/$USER/work-pac"  >> $DIR/tmp/blast-$SITE.sh
  echo "source $PACMAN/setup.sh"  >> $DIR/tmp/blast-$SITE.sh
  echo "$PACMAN/bin/pacman -clear-lock" >> $DIR/tmp/blast-$SITE.sh
  echo "$PACMAN/bin/pacman -trust-all-caches -get http://www.ci.uchicago.edu/~houzx/pac-cache:blast-$Signature"  >> $DIR/tmp/blast-$SITE.sh

  #make a timestamp for the deployment log
  echo "Starting `date`" >>$DIR/tmp/blast-deploy-$SITE-out.log
  echo "Starting `date`" >>$DIR/tmp/blast-deploy-$SITE-err.log

  # To transmit and execute the pacman install script to the given grid site
  #globus-job-run  $SITE  -stage $DIR/tmp/blast-$SITE.sh
  globus-url-copy  file://$DIR/tmp/blast-$SITE.sh  gsiftp://$SITE$APP/$VO/$USER/blast-$SITE.sh && globus-job-run  $SITE  /bin/sh $APP/$VO/$USER/blast-$SITE.sh 1>>$DIR/tmp/blast-deploy-$SITE-out.log 2>>$DIR/tmp/blast-deploy-$SITE-err.log && echo "Ending `date`" >>$DIR/tmp/blast-deploy-$SITE-out.log &
...
echo "Please see the standard output and error messages in $DIR/tmp/blast-deploy-SITE-out.log $DIR/tmp/blast-deploy-SITE-err.log"
