...
do
  ...
  echo "#/bin/sh" > $DIR/tmp/R-$SITE.sh
  echo "cd  $APP/$VO/$USER/work-pac"  >> $DIR/tmp/R-$SITE.sh
  echo "export PATH=$APP/$VO/$USER/work-pac/gcc-3.4.3/bin:/usr/bin:$PATH " >> $DIR/tmp/R-$SITE.sh
  echo "export LD_LIBRARY_PATH=$APP/$VO/$USER/work-pac/gcc-3.4.3/lib:/usr/lib:$LD_LIBRARY_PATH " >> $DIR/tmp/R-$SITE.sh
  echo "source $PACMAN/setup.sh"  >> $DIR/tmp/R-$SITE.sh
  echo "$PACMAN/bin/pacman -clear-lock" >> $DIR/tmp/R-$SITE.sh
  echo "$PACMAN/bin/pacman -trust-all-caches -get http://www.ci.uchicago.edu/~houzx/pac-cache:R"  >> $DIR/tmp/R-$SITE.sh

  #make a timestamp for the deployment log
  echo "Starting `date`" >>$DIR/tmp/R-deploy-$SITE-out.log
  echo "Starting `date`" >>$DIR/tmp/R-deploy-$SITE-err.log

  # To transmit and execute the pacman install script to the given grid site
  #globus-job-run  $SITE  -stage $DIR/tmp/R-$SITE.sh
  globus-url-copy  file://$DIR/tmp/R-$SITE.sh  gsiftp://$SITE$APP/$VO/$USER/R-$SITE.sh && globus-job-run  $SITE  /bin/sh $APP/$VO/$USER/R-$SITE.sh 1>>$DIR/tmp/R-deploy-$SITE-out.log 2>>$DIR/tmp/R-deploy-$SITE-err.log && echo "Ending `date`" >>$DIR/tmp/R-deploy-$SITE-out.log &

  #to delete the tempory deploy file
#  /bin/rm -f $DIR/tmp/R-$SITE.sh
  i=`expr $i + 1` 
done

echo "Please see the standard output and error messages in $DIR/tmp/R-deploy-SITE-out.log $DIR/tmp/R-deploy-SITE-err.log"
