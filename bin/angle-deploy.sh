...
do
  ...
  echo "#/bin/sh" > $DIR/tmp/angle-$SITE.sh
  echo "cd  $APP/$VO/$USER/work-pac"  >> $DIR/tmp/angle-$SITE.sh
  echo "source $PACMAN/setup.sh"  >> $DIR/tmp/angle-$SITE.sh
  echo "$PACMAN/bin/pacman -clear-lock" >> $DIR/tmp/angle-$SITE.sh
  echo "$PACMAN/bin/pacman -trust-all-caches -get http://www.ci.uchicago.edu/~houzx/pac-cache:angle"  >> $DIR/tmp/angle-$SITE.sh

  #make a timestamp for the deployment log
  echo "Starting `date`" >>$DIR/tmp/angle-deploy-$SITE-out.log
  echo "Starting `date`" >>$DIR/tmp/angle-deploy-$SITE-err.log

  # To transmit and execute the pacman install script to the given grid site
  #globus-job-run  $SITE  -stage $DIR/tmp/angle-$SITE.sh
  globus-url-copy  file://$DIR/tmp/angle-$SITE.sh  gsiftp://$SITE$APP/$VO/$USER/angle-$SITE.sh && globus-job-run  $SITE  /bin/sh $APP/$VO/$USER/angle-$SITE.sh 1>>$DIR/tmp/angle-deploy-$SITE-out.log 2>>$DIR/tmp/angle-deploy-$SITE-err.log && echo "Ending `date`" >>$DIR/tmp/angle-deploy-$SITE-out.log & 
  
  #to delete the tempory deploy file
  #/bin/rm -f $DIR/tmp/angle-$SITE.sh
  i=`expr $i + 1` 
done
echo "The standard output and error messge are respectively in $DIR/tmp/angle-deploy-SITE-out.log and $DIR/tmp/angle-deploy-SITE-err.log "
