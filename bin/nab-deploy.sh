...
do
  ...
  echo "#/bin/sh" > $DIR/tmp/nab-$SITE.sh
  echo "cd  $APP/$VO/$USER/work-pac"  >> $DIR/tmp/nab-$SITE.sh
  echo "export PATH=$APP/$VO/$USER/work-pac/gcc-3.4.3/bin:$APP/$VO/$USER/work-pac/flex-2.5.4/bin:$APP/$VO/$USER/work-pac/byacc-20070509/bin:/usr/bin:$PATH " >> $DIR/tmp/nab-$SITE.sh
  echo "export LD_LIBRARY_PATH=$APP/$VO/$USER/work-pac/gcc-3.4.3/lib:/usr/lib:$LD_LIBRARY_PATH " >> $DIR/tmp/nab-$SITE.sh
  echo "source $PACMAN/setup.sh"  >> $DIR/tmp/nab-$SITE.sh
  echo "$PACMAN/bin/pacman -clear-lock" >> $DIR/tmp/nab-$SITE.sh
  echo "$PACMAN/bin/pacman -trust-all-caches -get http://www.ci.uchicago.edu/~houzx/pac-cache:nab"  >> $DIR/tmp/nab-$SITE.sh

  #make a timestamp for the deployment log
  echo "Starting `date`" >>$DIR/tmp/nab-deploy-$SITE-out.log
  echo "Starting `date`" >>$DIR/tmp/nab-deploy-$SITE-err.log

  # To transmit and execute the pacman install script to the given grid site
  #globus-job-run  $SITE  -stage $DIR/tmp/nab-$SITE.sh
  globus-url-copy  file://$DIR/tmp/nab-$SITE.sh  gsiftp://$SITE$APP/$VO/$USER/nab-$SITE.sh && globus-job-run  $SITE  /bin/sh $APP/$VO/$USER/nab-$SITE.sh 1>>$DIR/tmp/nab-deploy-$SITE-out.log 2>>$DIR/tmp/nab-deploy-$SITE-err.log && echo "Ending `date`" >> $DIR/tmp/nab-deploy-$SITE-out.log & 
  
  #to delete the tempory deploy file
  #  /bin/rm -f $DIR/tmp/nab-$SITE.sh
  i=`expr $i + 1` 
done
echo "The standard output and error messge are respectively in $DIR/tmp/nab-deploy-SITE-out.log and $DIR/tmp/nab-deploy-SITE-err.log "
