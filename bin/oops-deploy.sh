...
do
  ...
  if [  -f $DIR/tmp/oops-$SITE.sh ]; then
     rm -f $DIR/tmp/oops-$SITE.sh
  fi
  echo "#/bin/sh" >> $DIR/tmp/oops-$SITE.sh
  echo "cd  $APP/$VO/$USER/work-pac"  >> $DIR/tmp/oops-$SITE.sh
  echo "source $PACMAN/setup.sh"  >> $DIR/tmp/oops-$SITE.sh
  echo "$PACMAN/bin/pacman -clear-lock" >> $DIR/tmp/oops-$SITE.sh
  echo "$PACMAN/bin/pacman -trust-all-caches -get http://www.ci.uchicago.edu/~houzx/pac-cache:oops"  >> $DIR/tmp/oops-$SITE.sh

  #make a timestamp for the deployment log
  echo "Starting `date`" >>$DIR/tmp/oops-deploy-$SITE-out.log
  echo "Starting `date`" >>$DIR/tmp/oops-deploy-$SITE-err.log

  # To transmit and execute the pacman install script to the given grid site
  #globus-job-run  $SITE  -stage $DIR/tmp/oops-$SITE.sh
  globus-url-copy  file://$DIR/tmp/oops-$SITE.sh  gsiftp://$SITE$APP/$VO/$USER/oops-$SITE.sh && globus-job-run  $SITE  /bin/sh $APP/$VO/$USER/oops-$SITE.sh 1>>$DIR/tmp/oops-deploy-$SITE-out.log 2>>$DIR/tmp/oops-deploy-$SITE-err.log && echo "Ending `date`" >>$DIR/tmp/oops-deploy-$SITE-out.log &
