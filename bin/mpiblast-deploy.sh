...
do
  ...
  echo "#/bin/sh" > $DIR/tmp/mpiblast-$SITE.sh
  echo "cd  $APP/$VO/$USER/work-pac"  >> $DIR/tmp/mpiblast-$SITE.sh
  echo "source $PACMAN/setup.sh"  >> $DIR/tmp/mpiblast-$SITE.sh
  echo "$PACMAN/bin/pacman -clear-lock" >> $DIR/tmp/mpiblast-$SITE.sh
  echo "$PACMAN/bin/pacman -trust-all-caches -get http://www.ci.uchicago.edu/~houzx/pac-cache:mpiblast"  >> $DIR/tmp/mpiblast-$SITE.sh

  #make a timestamp for the deployment log
  echo "Starting `date`" >>$DIR/tmp/mpiblast-deploy-$SITE-out.log
  echo "Starting `date`" >>$DIR/tmp/mpiblast-deploy-$SITE-err.log

  # To transmit and execute the pacman install script to the given grid site
  #globus-job-run  $SITE  -stage $DIR/tmp/mpiblast-$SITE.sh
  globus-url-copy  file://$DIR/tmp/mpiblast-$SITE.sh  gsiftp://$SITE$APP/$VO/$USER/mpiblast-$SITE.sh && globus-job-run  $SITE  /bin/sh $APP/$VO/$USER/mpiblast-$SITE.sh 1>>$DIR/tmp/mpiblast-deploy-$SITE-out.log 2>>$DIR/tmp/mpiblast-deploy-$SITE-err.log && echo "Ending `date`" >>$DIR/tmp/mpiblast-deploy-$SITE-out.log & 
  
  #to delete the tempory deploy file
  #/bin/rm -f $DIR/tmp/mpiblast-$SITE.sh
  i=`expr $i + 1` 
done
echo "The standard output and error messge are respectively in $DIR/tmp/mpiblast-deploy-SITE-out.log and $DIR/tmp/mpiblast-deploy-SITE-err.log "
